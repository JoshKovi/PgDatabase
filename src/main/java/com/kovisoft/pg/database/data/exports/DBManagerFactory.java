...
public class DBManagerFactory {
    ...
    public static DBManager createDBManager(DBManagerConfig config, AbstractMigration tm) throws SQLException, InterruptedException {
        String dbName = config.getDb();
        if(MANAGER_MAP.containsKey(dbName)) return MANAGER_MAP.get(dbName);
        DBManager manager = new DBManagerImpl(config, tm);
        MANAGER_MAP.put(dbName, manager);
        return manager;
    }
     ...
     *           you db tables. See {@link AbstractMigration} for more info. All migrations are flattened with
     *                   {@link CombinedMigration} which could result in conflicts if not managed carefully.
     */
    public static void overallSetupDB(DBManagerConfig config, List<AbstractMigration> migrations) throws SQLException, InterruptedException {
        CombinedMigration cm = new CombinedMigration(config.getDb() + "_archive", config.getDb(), migrations);
        DBManager manager = new DBManagerImpl(config, cm);
